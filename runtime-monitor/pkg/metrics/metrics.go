...
var (
	...
	RabbitAddEventsSuccessCount = addToCommon(prometheus.NewCounter(prometheus.CounterOpts{
		Name: "rabbit_add_events_success_count",
		Help: "Number of events successfully added to the rabbitmq queue",
	}))
	RabbitAddEventsFailureCount = addToCommon(prometheus.NewCounter(prometheus.CounterOpts{
		Name: "rabbit_add_events_failure_count",
		Help: "Number of events that failed to be added to the rabbitmq queue",
	}))
	TetragonEventsBufferSizeGauge = addToCommon(prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "tetragon_events_buffer_size_gauge",
		Help: "Number of events in the buffer received from the tetragon",
	}))
	TetragonEventsDroppedCount = addToCommon(prometheus.NewCounter(prometheus.CounterOpts{
		Name: "tetragon_events_dropped_count",
		Help: "Number of tetragon events dropped",
	}))
