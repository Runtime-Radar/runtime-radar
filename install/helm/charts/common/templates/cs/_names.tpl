...
{{- default (.Values.prometheus).externalHost ((.Values.global).prometheus).externalHost | default (printf "%s://prometheus.%s.svc.cluster.local:3000" (include "common.cs.prometheus.http-scheme" .) .Release.Namespace) | trimSuffix "/" }}
{{- end -}}

{{- define "common.cs.loki.address" -}}
{{- default (.Values.loki).externalHost ((.Values.global).loki).externalHost | default (printf "%s://loki.%s.svc.cluster.local:3000" (include "common.cs.loki.http-scheme" .) .Release.Namespace) | trimSuffix "/" }}
{{- end -}}
