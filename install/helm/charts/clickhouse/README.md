...
| `global.clickhouse.auth.existingSecret` | Name of an existing secret that contains the auth credentials (overrides `auth.existingSecret`) | `""`  |
...
| `auth.existingSecretPasswordKey` | Name of the key inside the existing secret. When empty, defaults to the container-env-style key `CLICKHOUSE_PASSWORD`                                                                                                                                                                                                               | `""`                         |
| `auth.existingSecretUserKey`     | Name of the secret key holding the ClickHouse username. When empty, defaults to the container-env-style key `CLICKHOUSE_USER`.                                                                                                                                                                                                      | `""`                         |
| `auth.existingSecretDatabaseKey` | Name of the secret key holding the ClickHouse database name. When empty, defaults to the container-env-style key `CLICKHOUSE_DB`. Note: the database name is read from the secret via `secretKeyRef` and used as-is; database names containing special characters must be stored already-escaped (no backtick-escaping is applied). | `""`                         |
