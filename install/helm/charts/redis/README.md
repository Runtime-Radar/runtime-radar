...
| `global.redis.auth.existingSecret` | Name of an existing secret that contains the auth credentials (overrides `auth.existingSecret`) | `""`  |
...
| `auth.existingSecretPasswordKey` | Password key to be retrieved from existing secret. When empty, defaults to the container-env-style key `REDIS_PASSWORD`   | `""`                    |
| `auth.existingSecretUserKey`     | Name of the secret key holding the Redis&reg; username. When empty, defaults to the container-env-style key `REDIS_USER`. | `""`                    |
