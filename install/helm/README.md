...
The chart-generated `cs-crt` certificate is re-issued on every upgrade and all RR components are restarted to pick it up, so the certificate always lists the components of the installed release in its subject alternative names. Set `global.tls.force=false` to keep the existing certificate. A certificate you provide yourself (`global.tls.existingSecret` or inline `tls.cert`/`tls.certKey`/`tls.certCA`) is never re-issued: add new component names, e.g. `kube-manager` in 0.3.0, to its subject alternative names before upgrading, otherwise the reverse proxy gets `502 Bad Gateway` from them.
...
- **Tetragon gRPC moved from TCP to a Unix socket.** `runtime-monitor.tetragon.grpc.address` now defaults to `unix:///var/run/tetragon/tetragon.sock` instead of `localhost:54321`. The old TCP listener ran on the host network without authentication, so any `hostNetwork` pod on the node could call privileged Tetragon RPCs (CVE-2026-65960). TCP addresses are no longer supported or tested — the chart schema rejects any address that does not start with `unix://`. The socket directory is fixed at `/var/run/tetragon`: it is the `tetragon-run` hostPath, mounted into both the Tetragon container and the runtime-monitor container, so only the socket file name can be changed.
- **runtime-monitor now needs group root and a `tetragon-run` mount.** Tetragon creates the socket as `root:root` mode `0660`, so the chart now ships defaults for the runtime-monitor DaemonSet where it previously shipped none: `runtime-monitor.containerSecurityContext` gets the repo-standard hardened block (the same one as history-api/kube-manager/notifier/policy-enforcer: `privileged: false`, `allowPrivilegeEscalation: false`, `readOnlyRootFilesystem: true`, `seccompProfile.type: RuntimeDefault`, `seLinuxOptions: {}`) plus `runAsGroup: 0`; `runtime-monitor.podSecurityContext` gets `enabled: true` and `supplementalGroups: [0]` (the devMode fallback, where the container-level securityContext is not rendered); and a top-level `runtime-monitor.volumeMounts` entry mounts `tetragon-run` read-only at `/var/run/tetragon` — the only mount that gives the app container the socket. Note that `readOnlyRootFilesystem: true` is a new constraint on a container that previously ran without any securityContext. Two consequences: admission policies that forbid GID 0 need an exception for this DaemonSet; and overlays that set `runtime-monitor.volumeMounts`, `runtime-monitor.podSecurityContext` or `runtime-monitor.containerSecurityContext` (all previously empty or absent) now silently clobber socket access — **merge with the chart defaults, do not replace them**.
...
| `global.tls.force`                      | Re-issue the generated TLS certificate and restart RR components on every upgrade                                                                                                                                                                                                                                                                                                                               | `true`        |
...
` |
| `runtime-monitor.tetragon.grpc.address`             | Set address of the Tetragon gRPC connection. Only Unix sockets on the tetragon-run hostPath (unix:///path/to/socket) are supported. | `unix:///var/run/tetragon/tetragon.sock`                                                                                              |
