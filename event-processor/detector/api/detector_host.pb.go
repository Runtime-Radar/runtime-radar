...
func NewDetectorPlugin(ctx context.Context, opts ...wazeroConfigOption) (*DetectorPlugin, error) {
	o := &WazeroConfig{
...
func (p *DetectorPlugin) LoadPath(ctx context.Context, pluginPath string) (detector, error) {
...
func (p *DetectorPlugin) LoadReader(ctx context.Context, reader io.Reader) (detector, error) {
...
func (p *DetectorPlugin) LoadBinary(ctx context.Context, b []byte) (detector, error) {
	...
	apiVersion := module.ExportedFunction("detector_api_version")
	if apiVersion == nil {
		return nil, errors.New("detector_api_version is not exported")
	}
	results, err := apiVersion.Call(ctx)
	if err != nil {
		return nil, err
	} else if len(results) != 1 {
		return nil, errors.New("invalid detector_api_version signature")
	}
	if results[0] != DetectorPluginAPIVersion {
		return nil, fmt.Errorf("API version mismatch, host: %d, plugin: %d", DetectorPluginAPIVersion, results[0])
	}

	info := module.ExportedFunction("detector_info")
	if info == nil {
		return nil, errors.New("detector_info is not exported")
	}
	detect := module.ExportedFunction("detector_detect")
	if detect == nil {
...
func (p *detectorPlugin) Close(ctx context.Context) (err error) {
...
func (p *detectorPlugin) Info(ctx context.Context, request *InfoReq) (*InfoResp, error) {
	...
	ptrSize, err := p.info.Call(ctx, dataPtr, dataSize)
	if err != nil {
		return nil, err
	}
...
func (p *detectorPlugin) Detect(ctx context.Context, request *DetectReq) (*DetectResp, error) {
	data, err := request.MarshalVT()
	if err != nil {
		return nil, err
	}
	...
	ptrSize, err := p.detect.Call(ctx, dataPtr, dataSize)
	if err != nil {
		return nil, err
	}
