...
const (
	...
	Description = "The detector detects if the standard input stream was redirected to a network socket, which may be a sign of reverse shell creation."
	Version     = 1
...
const (
	KprobeNoArgs  = "Detected that a reverse shell was started by the `%s` process"
	KprobeDefault = "Detected that a reverse shell was started by the `%s` process, which was started using the `%s` arguments"
)
