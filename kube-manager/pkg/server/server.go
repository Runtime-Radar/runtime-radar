...
func newGWMux(ctx context.Context, grpcAddr string, tlsConfig *tls.Config) (*runtime.ServeMux, error) {
	...
	if err := api.RegisterPodControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}
	if err := api.RegisterNodeControllerHandlerFromEndpoint(ctx, m, grpcAddr, opts); err != nil {
		return nil, err
	}
